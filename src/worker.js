...
export default {async fetch(request,env){const url=new URL(request.url);if(!env.WORKSHOP_PASSWORD||!env.SESSION_SECRET)return response('Der Workshop wird gerade eingerichtet.',503);
 ...
 if(url.pathname==='/login'&&request.method==='POST'){
  ...
  if(!allowed.success)return response(login('Zu viele Versuche. Bitte warte eine Minute.'),429,{'Content-Type':'text/html; charset=utf-8','Retry-After':'60'});
  if(!(await equal(password,env.WORKSHOP_PASSWORD)))return response(login('Das Passwort stimmt noch nicht. Versuch es erneut.'),401,{'Content-Type':'text/html; charset=utf-8'});
  return response(null,303,{'Location':'/','Set-Cookie':`${cookieName}=${await token(env.SESSION_SECRET)}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=43200`});
 }
 ...
 if(!(await validToken(cookie,env.SESSION_SECRET))){if(url.pathname==='/'||url.pathname==='/login')return response(request.method==='HEAD'?null:login(),200,{'Content-Type':'text/html; charset=utf-8'});return response('Bitte öffne den Workshop und melde dich an.',401)}
