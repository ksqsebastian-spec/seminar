...
def walkthroughs(modules):
 ...
 for i,m in enumerate(modules,1):
  result[i]=[
   frame('Hier beginnt deine Aufgabe.','Öffne ChatGPT. In das große Eingabefeld schreibst du gleich, was du erledigen möchtest. Du brauchst dafür keine besonderen Befehle.','work',focus='composer'),
   frame('Gib ChatGPT die passenden Unterlagen.','Lade die Übungsdatei über das Plus am Eingabefeld hoch. Warte, bis der Dateiname erscheint. So weißt du, dass ChatGPT die richtigen Unterlagen bekommen hat.','compose',file=m['file'],focus='upload'),
   ...
   frame('Jetzt siehst du ein mögliches Ergebnis.','Lies die Antwort und vergleiche sie mit deiner Ausgangsdatei. ChatGPT darf fehlende Angaben nicht einfach ergänzen.','result',prompt=m['prompt'],result=m['output'],kind=m['kind']),
 ...
 result[2]=[
  frame('Du musst nicht von vorn anfangen.','Wenn eine Antwort nicht passt, bleibst du in derselben Aufgabe. ChatGPT kennt dort bereits deine Anfrage und den ersten Entwurf.','result',result=modules[1]['input'],kind='mail'),
  ...
  frame('Wenn ChatGPT hängen bleibt: einen Schritt zurück.','Hat ChatGPT die Datei falsch gelesen, nenne die konkrete Stelle und lade die richtige Datei erneut hoch. Bleibt eine Browseraktion hängen, lass dir den aktuellen Stand zeigen und gib nur den nächsten Schritt vor.','compose',prompt='Stopp. Zeige mir kurz, was bereits erledigt ist und wo es hängt. Noch nichts erneut absenden. Ich gebe dir dann den nächsten Schritt.',focus='composer')
 ]
 result[3]=[
  ...
  frame('So könnte deine Vorgabe aussehen.','Für Lindenhof soll ChatGPT kurze Kundenantworten schreiben und unbekannte Angaben offenlassen. Speichere diese Vorgabe im Projekt; sie ist kein neuer Arbeitsauftrag.','instructions',prompt='Wir bearbeiten den Kundenauftrag Lindenhof. Schreibe Kundenantworten freundlich und kurz. Erfinde keine Maße, Preise oder Termine. Kennzeichne fehlende Angaben als „noch offen“.',focus='save'),
  frame('Starte jetzt eine Aufgabe in ChatGPT.','Öffne dein Projekt und wähle ChatGPT. Prüfe, dass Lindenhof als Projekt zugeordnet ist. Dann fragst du nach einer Zusammenfassung der Unterlagen.','compose',project='Lindenhof · Umbau',file='03-lindenhof.txt',prompt=modules[2]['prompt'],focus='project'),
 ...
 result[4]=[
  ...
  frame('Wähle das passende Programm.','Für eine gespeicherte Datei brauchst du OneDrive. Für E-Mails brauchst du Outlook Mail. Die Erweiterung bestimmt, in welchem verbundenen Programm ChatGPT nachsehen kann.','plugins',focus='onedrive'),
  frame('Nenne, was ChatGPT finden soll.','Beschreibe den Ordner und die gesuchte Datei möglichst genau. Du musst sie nicht erst selbst herunterladen und wieder hochladen.','compose',prompt=modules[3]['prompt'],focus='send'),
 ...
 result[5]=[
  frame('Lass dir zuerst die heutigen Mails zeigen.','Öffne ChatGPT mit deiner Outlook-Verbindung. Bitte zunächst nur um Absender und Betreff. Danach entscheidest du, welche Mail du bearbeiten möchtest.','compose',prompt=modules[4]['mailStages'][0]['prompt'],focus='send'),
  ...
  frame('Lass dir erklären, was gebraucht wird.','ChatGPT soll die Mail in einfachen Worten zusammenfassen. Frag zusätzlich, was du selbst als Nächstes tun musst.','result',prompt=modules[4]['mailStages'][1]['prompt'],result=modules[4]['mailStages'][1]['result'],kind='notes'),
  frame('Gib ChatGPT die Fakten für die Antwort.','ChatGPT kann nicht wissen, welche Unterlagen dein Betrieb benötigt. Hier gibst du vor, dass ein Grundriss und Fotos gebraucht werden. Daraus entsteht zunächst ein Vorschlag in ChatGPT.','result',prompt=modules[4]['mailStages'][2]['prompt'],result=modules[4]['mailStages'][2]['result'],kind='mail'),
  ...
  frame('Kontrolliere den Entwurf in Outlook.','Öffne Outlook und den Ordner Entwürfe. Lies Empfänger und Antwort durch. Als Zusatz kannst du ChatGPT eine Übungsdatei unter 3 MB an diesen Entwurf anhängen lassen; prüfe den Anhang danach ebenfalls.','outlook',result=modules[4]['output'])
 ]
 result[6]=[
  frame('Beginne mit einem klaren Suchauftrag.','Öffne ChatGPT und nenne Website, Ort, Budget und die Kriterien, die wirklich wichtig sind. Sag auch, ob ChatGPT nur recherchieren oder etwas auf der Website ausführen soll.','compose',prompt=modules[5]['prompt'],focus='send'),
  frame('Lass dir die geöffnete Website zeigen.','Wenn ChatGPT den Browser verwendet, kannst du dich auf die sichtbare Seite beziehen. Beschreibe das Ziel des nächsten Schritts statt eine lange Folge von Klicks vorzugeben.','browser'),
  frame('Steuere in kleinen Schritten nach.','Fehlt zum Beispiel die Lagerfläche, sag das direkt in derselben Aufgabe. ChatGPT soll den Treffer prüfen, statt eine fehlende Angabe zu erraten.','compose',prompt='Prüfe beim zweiten Objekt, ob eine Lagerfläche dazugehört. Zeige mir die Fundstelle. Wenn es dort nicht steht, kennzeichne die Angabe als unklar. Kontaktiere keinen Anbieter.',focus='send'),
 ...
 result[7][1]['body']='Für den ersten Weg öffnest du das Plus in ChatGPT und lädst „04-materialvergleich.xlsx“ hoch. Achte darauf, dass genau diese Datei als Anhang am Auftrag steht.'
 result[7][3]['body']='ChatGPT rechnet hier Menge mal Stückpreis und addiert die Lieferung. Dadurch ist Angebot B insgesamt günstiger, obwohl der Stückpreis höher ist als bei A.'
 ...
 result[9][1]=frame('Nenne Anlass, Dauer und Beteiligte.','ChatGPT braucht genug Angaben, um einen sinnvollen Vorschlag zu machen. Für unbekannte Daten soll es nachfragen. Ein freier Platz in deinem Kalender sagt nichts über die Kundin aus.','compose',prompt=modules[8]['prompt'],focus='send')
